...
impl<'a> Parser<'a> {
    ...
    fn parse_prototype(&mut self) -> Result<Prototype, &'static str> {
        ...
        match self.curr() {
            LParen => (),
            _ => {
                return Err("Expected '(' character in prototype declaration.")
            }
        }
            ...
            LParen => {
                ...
                loop {
                    ...
                    match self.current()? {
                        ...
                        _ => {
                            return Err(
                                "Expected ',' character in function call.",
                            )
                        }
                    }
    ...
    fn parse_var_expr(&mut self) -> Result<Expr, &'static str> {
        ...
        loop {
            let name = match self.curr() {
                Ident(name) => name,
                _ => {
                    return Err("Expected identifier in 'var..in' declaration.")
                }
            };
            ...
            match self.curr() {
                ...
                _ => return Err(
                    "Expected comma or 'in' keyword in variable declaration.",
                ),
            }
        }
    ...
    fn compile_expr(
        ...
        match *expr {
            ...
            Expr::Conditional {
                ...
                let float_conversion: Result<FloatValue, _> =
                    phi.as_instruction_value().try_into();

                match float_conversion {
                    Ok(v) => Ok(v),
                    _ => Err("Failed to convert PhiNode to FloatValue."),
                }
            }
...
pub fn main() {
    ...
    loop {
            ...
            Ok(fun) => {
                ...
                match Compiler::compile(&context, &builder, &fpm, &module, &fun)
                {
                    Ok(function) => {
                        ...
                        (
                            function.get_name().to_str().unwrap().to_string(),
                            is_anon,
                        )
                    }
                    Err(err) => {
        ...
        if is_anonymous {
            ...
            let maybe_fn = unsafe {
                ee.get_function::<unsafe extern "C" fn() -> f64>(name.as_str())
            };
