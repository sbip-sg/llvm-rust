...
use llvm_sys::core::{
    LLVMAppendBasicBlockInContext, LLVMConstStringInContext,
    LLVMConstStructInContext, LLVMContextCreate, LLVMContextDispose,
    LLVMContextSetDiagnosticHandler, LLVMCreateBuilderInContext,
    LLVMDoubleTypeInContext, LLVMFP128TypeInContext, LLVMFloatTypeInContext,
    LLVMGetGlobalContext, LLVMGetMDKindIDInContext, LLVMHalfTypeInContext,
    LLVMInsertBasicBlockInContext, LLVMInt16TypeInContext,
    LLVMInt1TypeInContext, LLVMInt32TypeInContext, LLVMInt64TypeInContext,
    LLVMInt8TypeInContext, LLVMIntTypeInContext, LLVMMDNodeInContext,
...
use llvm_sys::prelude::{
    LLVMContextRef, LLVMDiagnosticInfoRef, LLVMTypeRef, LLVMValueRef,
};
...
use crate::types::{
    AnyTypeEnum, AsTypeRef, BasicTypeEnum, FloatType, FunctionType, IntType,
    StructType, VoidType,
};
use crate::values::{
...
impl Context {
    ...
    pub fn ptr_sized_int_type(
        ...
        let int_type_ptr = match address_space {
            ...
            None => unsafe {
                LLVMIntPtrTypeInContext(self.context, target_data.target_data)
            },
        };
    ...
    pub fn insert_basic_block_after(
        ...
        match basic_block.get_next_basic_block() {
            Some(next_basic_block) => {
                self.prepend_basic_block(next_basic_block, name)
            }
            None => {
