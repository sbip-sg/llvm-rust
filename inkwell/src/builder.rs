...
use llvm_sys::core::{
    LLVMAddCase, LLVMAddClause, LLVMAddDestination, LLVMBuildAShr,
    LLVMBuildAdd, LLVMBuildAddrSpaceCast, LLVMBuildAggregateRet,
    LLVMBuildAlloca, LLVMBuildAnd, LLVMBuildArrayAlloca, LLVMBuildArrayMalloc,
    LLVMBuildAtomicRMW, LLVMBuildBitCast, LLVMBuildBr, LLVMBuildCall,
    LLVMBuildCast, LLVMBuildCondBr, LLVMBuildExactSDiv,
    LLVMBuildExtractElement, LLVMBuildExtractValue, LLVMBuildFAdd,
    LLVMBuildFCmp, LLVMBuildFDiv, LLVMBuildFMul, LLVMBuildFNeg,
    LLVMBuildFPCast, LLVMBuildFPExt, LLVMBuildFPToSI, LLVMBuildFPToUI,
    LLVMBuildFPTrunc, LLVMBuildFRem, LLVMBuildFSub, LLVMBuildFence,
    LLVMBuildFree, LLVMBuildGEP, LLVMBuildGlobalString,
    LLVMBuildGlobalStringPtr, LLVMBuildICmp, LLVMBuildInBoundsGEP,
    LLVMBuildIndirectBr, LLVMBuildInsertElement, LLVMBuildInsertValue,
    LLVMBuildIntCast, LLVMBuildIntToPtr, LLVMBuildInvoke, LLVMBuildIsNotNull,
    LLVMBuildIsNull, LLVMBuildLShr, LLVMBuildLandingPad, LLVMBuildLoad,
    LLVMBuildMalloc, LLVMBuildMul, LLVMBuildNSWAdd, LLVMBuildNSWMul,
    LLVMBuildNSWNeg, LLVMBuildNSWSub, LLVMBuildNUWAdd, LLVMBuildNUWMul,
    LLVMBuildNUWNeg, LLVMBuildNUWSub, LLVMBuildNeg, LLVMBuildNot, LLVMBuildOr,
    LLVMBuildPhi, LLVMBuildPointerCast, LLVMBuildPtrDiff, LLVMBuildPtrToInt,
    LLVMBuildResume, LLVMBuildRet, LLVMBuildRetVoid, LLVMBuildSDiv,
    LLVMBuildSExt, LLVMBuildSExtOrBitCast, LLVMBuildSIToFP, LLVMBuildSRem,
    LLVMBuildSelect, LLVMBuildShl, LLVMBuildShuffleVector, LLVMBuildStore,
    LLVMBuildStructGEP, LLVMBuildSub, LLVMBuildSwitch, LLVMBuildTrunc,
    LLVMBuildTruncOrBitCast, LLVMBuildUDiv, LLVMBuildUIToFP, LLVMBuildURem,
    LLVMBuildUnreachable, LLVMBuildVAArg, LLVMBuildXor, LLVMBuildZExt,
    LLVMBuildZExtOrBitCast, LLVMClearInsertionPosition, LLVMDisposeBuilder,
    LLVMGetInsertBlock, LLVMInsertIntoBuilder, LLVMInsertIntoBuilderWithName,
    LLVMPositionBuilder, LLVMPositionBuilderAtEnd, LLVMPositionBuilderBefore,
    LLVMSetCleanup,
};
#[llvm_versions(8.0..=latest)]
use llvm_sys::core::{
    LLVMBuildIntCast2, LLVMBuildMemCpy, LLVMBuildMemMove, LLVMBuildMemSet,
};
...
use crate::values::{
    AggregateValue, AggregateValueEnum, AsValueRef, BasicMetadataValueEnum,
    BasicValue, BasicValueEnum, CallSiteValue, FloatMathValue, FunctionValue,
    GlobalValue, InstructionOpcode, InstructionValue, IntMathValue, IntValue,
...
impl<'ctx> Builder<'ctx> {
        ...
        F: Into<CallableValue<'ctx>>,
    {
        ...
        let mut args: Vec<LLVMValueRef> =
            args.iter().map(|val| val.as_value_ref()).collect();
        let value = unsafe {
            LLVMBuildCall(
                self.builder,
                fn_val_ref,
    ...
    /// }
    /// ```
    pub fn build_resume<V: BasicValue<'ctx>>(
        &self,
        value: V,
    ) -> InstructionValue<'ctx> {
        let val =
            unsafe { LLVMBuildResume(self.builder, value.as_value_ref()) };
    ...
    pub fn build_free(
        ...
        unsafe {
            InstructionValue::new(LLVMBuildFree(
                self.builder,
                ptr.as_value_ref(),
            ))
        }
    ...
    pub fn build_xor<T: IntMathValue<'ctx>>(
        &self,
        lhs: T,
        rhs: T,
        name: &str,
    ...
    // SubType: (&self, lhs: &IntValue<bool>, rhs: &IntValue<bool>, name: &str) -> IntValue<bool> {
    pub fn build_and<T: IntMathValue<'ctx>>(
        &self,
        lhs: T,
        rhs: T,
        name: &str,
    ...
    // SubType: (&self, lhs: &IntValue<bool>, rhs: &IntValue<bool>, name: &str) -> IntValue<bool> {
    pub fn build_or<T: IntMathValue<'ctx>>(
        &self,
        lhs: T,
        rhs: T,
        name: &str,
    ...
    pub fn build_unconditional_branch(
        ...
        let value =
            unsafe { LLVMBuildBr(self.builder, destination_block.basic_block) };
    ...
    pub fn build_int_neg<T: IntMathValue<'ctx>>(
        &self,
        value: T,
        name: &str,
    ) -> T {
    ...
    pub fn build_cmpxchg<V: BasicValue<'ctx>>(
        ...
        if ptr.get_type().get_element_type().to_basic_type_enum()
            != cmp.get_type()
        {
            return Err(
        ...
        if success < AtomicOrdering::Monotonic
        ...
        if failure == AtomicOrdering::Release
            || failure == AtomicOrdering::AcquireRelease
        {
            return Err(
