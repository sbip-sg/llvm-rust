...
use llvm_sys::bit_writer::{
    LLVMWriteBitcodeToFile, LLVMWriteBitcodeToMemoryBuffer,
};
use llvm_sys::core::{
    LLVMAddFunction, LLVMAddGlobal, LLVMAddGlobalInAddressSpace,
    LLVMAddNamedMetadataOperand, LLVMCloneModule, LLVMDisposeModule,
    LLVMDumpModule, LLVMGetFirstFunction, LLVMGetFirstGlobal,
    LLVMGetLastFunction, LLVMGetLastGlobal, LLVMGetModuleContext,
    LLVMGetNamedFunction, LLVMGetNamedGlobal, LLVMGetNamedMetadataNumOperands,
    LLVMGetNamedMetadataOperands, LLVMGetTarget, LLVMGetTypeByName,
    LLVMPrintModuleToFile, LLVMPrintModuleToString, LLVMSetDataLayout,
    LLVMSetTarget,
};
...
use crate::values::{
    AsValueRef, FunctionValue, GlobalValue, MetadataValue, PointerValue,
};
...
impl<'ctx> Module<'ctx> {
    pub(crate) unsafe fn new(module: LLVMModuleRef) -> Self {
        ...
        Module {
            ...
            data_layout: RefCell::new(Some(Module::get_borrowed_data_layout(
                module,
            ))),
            _marker: PhantomData,
        }
    ...
    pub fn add_function(
        ...
        let fn_value = unsafe {
            ...
            .expect(
                "add_function should always succeed in adding a new function",
            )
        };
    ...
    pub fn get_function(&self, name: &str) -> Option<FunctionValue<'ctx>> {
        ...
        unsafe {
            FunctionValue::new(LLVMGetNamedFunction(
                self.module.get(),
                c_string.as_ptr(),
            ))
        }
    ...
    pub fn get_struct_type(&self, name: &str) -> Option<StructType<'ctx>> {
        ...
        let struct_type =
            unsafe { LLVMGetTypeByName(self.module.get(), c_string.as_ptr()) };
    ...
    pub fn create_execution_engine(
        ...
        Target::initialize_native(&InitializationConfig::default()).map_err(
            |mut err_string| {
                err_string.push('\0');
        ...
        if self.owned_by_ee.borrow().is_some() {
            let string =
                "This module is already owned by an ExecutionEngine.\0";
        ...
        let execution_engine =
            unsafe { ExecutionEngine::new(Rc::new(execution_engine), false) };
    ...
    pub fn create_jit_execution_engine(
        ...
        let execution_engine = unsafe { execution_engine.assume_init() };
        let execution_engine =
            unsafe { ExecutionEngine::new(Rc::new(execution_engine), true) };
    ...
    pub fn write_bitcode_to_memory(&self) -> MemoryBuffer {
        let memory_buffer =
            unsafe { LLVMWriteBitcodeToMemoryBuffer(self.module.get()) };
    ...
    pub fn set_data_layout(&self, data_layout: &DataLayout) {
        ...
        *self.data_layout.borrow_mut() =
            Some(Module::get_borrowed_data_layout(self.module.get()));
    }
    ...
    pub fn print_to_file<P: AsRef<Path>>(
        &self,
        path: P,
    ) -> Result<(), LLVMString> {
        let path_str = path
    ...
    pub fn get_global(&self, name: &str) -> Option<GlobalValue<'ctx>> {
        let c_string = to_c_str(name);
        let value =
            unsafe { LLVMGetNamedGlobal(self.module.get(), c_string.as_ptr()) };
    ...
    /// An iterator over the global values in this `Module`.
    ///
    /// ```
    /// use inkwell::AddressSpace;
    /// use inkwell::context::Context;
    ...
    /// let global = module.add_global(i8_type, Some(AddressSpace::Const), "my_global");
    ///
    /// assert_eq!(module.get_globals().collect::<Vec<GlobalValue>>(), vec![global]);
    /// ```
    ...
    pub fn get_name(&self) -> &CStr {
        let mut length = 0;
        let cstr_ptr =
            unsafe { LLVMGetModuleIdentifier(self.module.get(), &mut length) };
    ...
    pub fn link_in_module(&self, other: Self) -> Result<(), LLVMString> {
        ...
        #[cfg(not(any(feature = "llvm3-6", feature = "llvm3-7")))]
        {
            ...
            let char_ptr_ptr = &mut char_ptr as *mut *mut ::libc::c_char
                as *mut *mut c_void
                as *mut c_void;
            ...
            context.set_diagnostic_handler(
                get_error_str_diagnostic_handler,
                char_ptr_ptr,
            );
