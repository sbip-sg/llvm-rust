...
use llvm_sys::debuginfo::{
    LLVMCreateDIBuilder, LLVMCreateDIBuilderDisallowUnresolved,
};
use llvm_sys::debuginfo::{
    ...
    LLVMDIBuilderCreateMemberType, LLVMDIBuilderCreateNameSpace,
    LLVMDIBuilderCreateParameterVariable, LLVMDIBuilderCreateStructType,
    LLVMDIBuilderCreateSubroutineType, LLVMDIBuilderCreateUnionType,
    LLVMDIBuilderFinalize, LLVMDIBuilderInsertDbgValueBefore,
    LLVMDIBuilderInsertDeclareAtEnd, LLVMDIBuilderInsertDeclareBefore,
    LLVMDILocationGetColumn, LLVMDILocationGetLine, LLVMDILocationGetScope,
    LLVMDISubprogramGetFlags, LLVMDITypeGetAlignInBits,
...
impl<'ctx> DebugInfoBuilder<'ctx> {
    ...
    pub fn create_struct_type(
        ...
        let derived_from =
            derived_from.map_or(std::ptr::null_mut(), |dt| dt.metadata_ref);
        let vtable_holder =
            vtable_holder.map_or(std::ptr::null_mut(), |dt| dt.metadata_ref);
        let metadata_ref = unsafe {
    ...
    pub fn create_subroutine_type(
        ...
        let mut p =
            vec![return_type.map_or(std::ptr::null_mut(), |t| t.metadata_ref)];
        p.append(
            &mut parameter_types
                .iter()
    ...
    pub fn create_global_variable_expression(
        ...
        let expression_ptr =
            expression.map_or(std::ptr::null_mut(), |dt| dt.metadata_ref);
        let decl_ptr =
            declaration.map_or(std::ptr::null_mut(), |dt| dt.metadata_ref);
        let metadata_ref = unsafe {
            LLVMDIBuilderCreateGlobalVariableExpression(
                self.builder,
                scope.metadata_ref,
    ...
    pub fn create_constant_expression(&self, value: i64) -> DIExpression<'ctx> {
        let metadata_ref = unsafe {
            LLVMDIBuilderCreateConstantValueExpression(self.builder, value as _)
        };
    ...
    pub fn create_expression(
        &self,
        mut address_operations: Vec<i64>,
    ) -> DIExpression<'ctx> {
        let metadata_ref = unsafe {
    ...
    pub unsafe fn create_placeholder_derived_type(
        ...
        let metadata_ref =
            LLVMTemporaryMDNode(context.context, std::ptr::null_mut(), 0);
        DIDerivedType {
            metadata_ref,
            _marker: PhantomData,
        }
...
/// Implementing functions for `DISubprogram`
impl<'a> DISubprogram<'a> {
    /// Get flags in `DISubprogram`
    /// To check the visibility of functions
    /// Notable examples are LLVMDIFlagPublic, LLVMDIFlagPrivate, LLVMDIFlagProtected
    pub fn get_flags(self) -> llvm_sys::debuginfo::LLVMDIFlags {
        return unsafe { LLVMDISubprogramGetFlags(self.metadata_ref) };
    }
}
...
mod flags {
    ...
    impl DIFlagsConstants for DIFlags {
        ...
        const OBJC_CLASS_COMPLETE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagObjcClassComplete;
        const OBJECT_POINTER: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagObjectPointer;
        const VECTOR: DIFlags = llvm_sys::debuginfo::LLVMDIFlagVector;
        const STATIC_MEMBER: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagStaticMember;
        const LVALUE_REFERENCE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagLValueReference;
        const RVALUE_REFERENCE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagRValueReference;
        const RESERVED: DIFlags = llvm_sys::debuginfo::LLVMDIFlagReserved;
        const SINGLE_INHERITANCE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagSingleInheritance;
        const MULTIPLE_INHERITANCE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagMultipleInheritance;
        const VIRTUAL_INHERITANCE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagVirtualInheritance;
        const INTRODUCED_VIRTUAL: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagIntroducedVirtual;
        ...
        const TYPE_PASS_BY_VALUE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagTypePassByValue;
        const TYPE_PASS_BY_REFERENCE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagTypePassByReference;
        //#[llvm_versions(7.0)]
        ...
        const INDIRECT_VIRTUAL_BASE: DIFlags =
            llvm_sys::debuginfo::LLVMDIFlagIndirectVirtualBase;
    }
