use llvm_sys::target::{
    ...
    LLVMCallFrameAlignmentOfType, LLVMCopyStringRepOfTargetData,
    LLVMCreateTargetData, LLVMDisposeTargetData, LLVMElementAtOffset,
    LLVMIntPtrTypeForASInContext, LLVMIntPtrTypeInContext, LLVMOffsetOfElement,
    LLVMPointerSize, LLVMPointerSizeForAS, LLVMPreferredAlignmentOfGlobal,
    LLVMPreferredAlignmentOfType, LLVMSizeOfTypeInBits, LLVMStoreSizeOfType,
    LLVMTargetDataRef,
};
#[llvm_versions(4.0..=latest)]
use llvm_sys::target_machine::LLVMCreateTargetDataLayout;
use llvm_sys::target_machine::{
    LLVMAddAnalysisPasses, LLVMCodeGenFileType, LLVMCodeGenOptLevel,
    LLVMCodeModel, LLVMCreateTargetMachine, LLVMDisposeTargetMachine,
    LLVMGetDefaultTargetTriple, LLVMGetFirstTarget, LLVMGetNextTarget,
    ...
    LLVMGetTargetMachineTarget, LLVMGetTargetMachineTriple, LLVMGetTargetName,
    LLVMRelocMode, LLVMSetTargetMachineAsmVerbosity, LLVMTargetHasAsmBackend,
    LLVMTargetHasJIT, LLVMTargetHasTargetMachine, LLVMTargetMachineEmitToFile,
...
impl Target {
    ...
    pub fn initialize_r600(config: &InitializationConfig) {
        use llvm_sys::target::{
            LLVMInitializeR600AsmParser, LLVMInitializeR600AsmPrinter,
            LLVMInitializeR600Target, LLVMInitializeR600TargetInfo,
    ...
    pub fn initialize_x_core(config: &InitializationConfig) {
        use llvm_sys::target::{
            ...
            LLVMInitializeXCoreTarget, LLVMInitializeXCoreTargetInfo,
    ...
    pub fn initialize_bpf(config: &InitializationConfig) {
        use llvm_sys::target::{
            LLVMInitializeBPFAsmPrinter, LLVMInitializeBPFTarget,
    ...
    pub fn initialize_native(
        ...
        if config.asm_parser {
            ...
            if code == 1 {
                ...
                return Err(
                    "Unknown error in initializing native asm parser".into()
                );
            }
        }
        ...
        if config.disassembler {
            ...
            if code == 1 {
                return Err(
                    "Unknown error in initializing native disassembler".into(),
                );
            }
        }
...
impl TargetMachine {
    ...
    pub fn get_triple(&self) -> TargetTriple {
        let str = unsafe {
            LLVMString::new(LLVMGetTargetMachineTriple(self.target_machine))
        };
    ...
    pub fn get_default_triple() -> TargetTriple {
        let llvm_string =
            unsafe { LLVMString::new(LLVMGetDefaultTargetTriple()) };
    ...
    pub fn normalize_triple(triple: &TargetTriple) -> TargetTriple {
        ...
        let normalized = unsafe {
            LLVMString::new(LLVMNormalizeTargetTriple(triple.as_ptr()))
        };
    ...
    pub fn get_feature_string(&self) -> &CStr {
        unsafe {
            CStr::from_ptr(LLVMGetTargetMachineFeatureString(
                self.target_machine,
            ))
        }
    }
...
impl TargetData {
    ...
    pub fn get_data_layout(&self) -> DataLayout {
        unsafe {
            DataLayout::new_owned(LLVMCopyStringRepOfTargetData(
                self.target_data,
            ))
        }
    }
    ...
    pub fn get_preferred_alignment_of_global(
        &self,
        value: &GlobalValue,
    ) -> u32 {
        unsafe {
