...
use llvm_sys::execution_engine::{
    LLVMAddGlobalMapping, LLVMAddModule, LLVMDisposeExecutionEngine,
    LLVMExecutionEngineRef, LLVMFindFunction, LLVMFreeMachineCodeForFunction,
    LLVMGenericValueRef, LLVMGetExecutionEngineTargetData,
    LLVMGetFunctionAddress, LLVMLinkInInterpreter, LLVMLinkInMCJIT,
    LLVMRemoveModule, LLVMRunFunction, LLVMRunFunctionAsMain,
    ...
    fn as_str(&self) -> &str {
        match self {
            ...
            FunctionLookupError::FunctionNotFound => {
                "Function not found in ExecutionEngine"
            }
        }
    }
}
...
impl RemoveModuleError {
    fn as_str(&self) -> &str {
        match self {
            RemoveModuleError::ModuleNotOwned => {
                "Module is not owned by an Execution Engine"
            }
...
impl<'ctx> ExecutionEngine<'ctx> {
    pub(crate) unsafe fn new(
        ...
        ExecutionEngine {
            execution_engine: Some(ExecEngineInner(
                execution_engine,
                PhantomData,
            )),
        ...
        F: UnsafeFunctionPointer,
    {
        ...
        let execution_engine =
            self.execution_engine.as_ref().expect(EE_INNER_PANIC);
    ...
    pub unsafe fn run_function(
        ...
        let mut args: Vec<LLVMGenericValueRef> =
            args.iter().map(|val| val.generic_value).collect();
    ...
    pub unsafe fn run_function_as_main(
        ...
        let cstring_args: Vec<_> =
            args.iter().map(|&arg| to_c_str(arg)).collect();
        let raw_args: Vec<*const _> =
            cstring_args.iter().map(|arg| arg.as_ptr()).collect();
...
struct ExecEngineInner<'ctx>(
    Rc<LLVMExecutionEngineRef>,
...
pub mod experimental {
    use llvm_sys::error::{
        LLVMConsumeError, LLVMErrorRef, LLVMErrorTypeId, LLVMGetErrorMessage,
        LLVMGetErrorTypeId,
    };
    use llvm_sys::orc::{
        LLVMOrcAddEagerlyCompiledIR, LLVMOrcAddLazilyCompiledIR,
        LLVMOrcCreateInstance, LLVMOrcDisposeInstance,
        LLVMOrcDisposeMangledSymbol, LLVMOrcGetErrorMsg,
    ...
    impl Orc {
        pub fn create(target_machine: TargetMachine) -> Self {
            let stack_ref =
                unsafe { LLVMOrcCreateInstance(target_machine.target_machine) };

            Orc(stack_ref)
        }

        pub fn add_compiled_ir<'ctx>(
            &self,
            module: &Module<'ctx>,
            lazily: bool,
        ) -> Result<(), ()> {
