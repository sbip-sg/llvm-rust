...
// TRUNG: allow all clippy lints in Inkwell
#![allow(clippy::all)]
// TRUNG: comment the following to report warnings of inkwell
#![allow(warnings)]
#![deny(missing_debug_implementations)]
#![cfg_attr(feature = "beta", feature(doc_cfg))]
//--------------------------------------------------------
...
use llvm_sys::{
    LLVMAtomicOrdering, LLVMAtomicRMWBinOp, LLVMDLLStorageClass,
    LLVMIntPredicate, LLVMRealPredicate, LLVMThreadLocalMode, LLVMVisibility,
};
...
impl ThreadLocalMode {
    pub(crate) fn new(thread_local_mode: LLVMThreadLocalMode) -> Option<Self> {
        match thread_local_mode {
            ...
            LLVMThreadLocalMode::LLVMLocalExecTLSModel => {
                Some(ThreadLocalMode::LocalExecTLSModel)
            }
