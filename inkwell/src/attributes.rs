...
use llvm_sys::core::{
    ...
    LLVMGetEnumAttributeValue, LLVMGetLastEnumAttributeKind,
...
impl Attribute {
    ...
    pub fn get_string_kind_id(&self) -> &CStr {
        ...
        let mut length = 0;
        let cstr_ptr =
            unsafe { LLVMGetStringAttributeKind(self.attribute, &mut length) };
    ...
    pub fn get_string_value(&self) -> &CStr {
        ...
        let mut length = 0;
        let cstr_ptr =
            unsafe { LLVMGetStringAttributeValue(self.attribute, &mut length) };
