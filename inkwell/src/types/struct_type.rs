...
use llvm_sys::core::{
    LLVMConstArray, LLVMConstNamedStruct, LLVMCountStructElementTypes,
    LLVMGetStructElementTypes, LLVMGetStructName, LLVMIsOpaqueStruct,
    LLVMIsPackedStruct, LLVMStructSetBody,
};
...
use crate::values::{
    ArrayValue, AsValueRef, BasicValueEnum, IntValue, StructValue,
};
...
impl<'ctx> StructType<'ctx> {
    ...
    /// ```
    #[llvm_versions(3.7..=latest)]
    pub fn get_field_type_at_index(
        self,
        index: u32,
    ...
    pub fn print_to_llvm_string(self) -> LLVMString {
        self.struct_type.print_to_llvm_string()
    }
