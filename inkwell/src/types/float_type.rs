use llvm_sys::core::{
    LLVMConstArray, LLVMConstReal, LLVMConstRealOfStringAndSize,
};
...
use crate::values::{
    ArrayValue, AsValueRef, FloatValue, GenericValue, IntValue,
};
...
impl<'ctx> FloatType<'ctx> {
    ...
    pub fn print_to_llvm_string(self) -> LLVMString {
        self.float_type.print_to_llvm_string()
    }
    ...
    pub fn create_generic_value(self, value: f64) -> GenericValue<'ctx> {
        unsafe {
            GenericValue::new(LLVMCreateGenericValueOfFloat(
                self.as_type_ref(),
                value,
            ))
        }
