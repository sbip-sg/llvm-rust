use llvm_sys::core::{
    LLVMConstAllOnes, LLVMConstArray, LLVMConstInt,
    LLVMConstIntOfArbitraryPrecision, LLVMConstIntOfStringAndSize,
    LLVMGetIntTypeWidth,
};
...
impl StringRadix {
    ...
    pub fn matches_str(&self, slice: &str) -> bool {
        ...
        match self {
            ...
            StringRadix::Hexadecimal => {
                it.all(|c| matches!(c, '0'..='9' | 'a'..='f' | 'A'..='F'))
            }
            StringRadix::Alphanumeric => {
                it.all(|c| matches!(c, '0'..='9' | 'a'..='z' | 'A'..='Z'))
            }
        }
    }
}
...
impl<'ctx> IntType<'ctx> {
    ...
    pub fn print_to_llvm_string(self) -> LLVMString {
        self.int_type.print_to_llvm_string()
    }
