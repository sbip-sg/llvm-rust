...
impl<'ctx> PointerType<'ctx> {
    ...
    pub fn get_address_space(self) -> AddressSpace {
        let addr_space =
            unsafe { LLVMGetPointerAddressSpace(self.as_type_ref()) };

        AddressSpace::try_from(addr_space)
            .expect("Unexpectedly found invalid AddressSpace value")
    }

    /// Print the definition of a `PointerType` to `LLVMString`.
    pub fn print_to_llvm_string(self) -> LLVMString {
        self.ptr_type.print_to_llvm_string()
    }
