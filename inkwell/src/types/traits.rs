...
use crate::types::{
    ArrayType, FloatType, FunctionType, IntType, PointerType, StructType, Type,
    VectorType, VoidType,
};
use crate::values::{
    FloatMathValue, FloatValue, IntMathValue, IntValue, PointerMathValue,
...
pub trait AnyType<'ctx>: AsTypeRef + Debug {
    ...
    fn print_to_llvm_string(&self) -> LLVMString {
        unsafe { Type::new(self.as_type_ref()).print_to_llvm_string() }
    }
