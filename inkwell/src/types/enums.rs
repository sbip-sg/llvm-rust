...
use crate::types::{
    ArrayType, FloatType, FunctionType, IntType, PointerType, StructType,
    VectorType, VoidType,
};
...
impl<'ctx> BasicMetadataTypeEnum<'ctx> {
    ...
    pub fn print_to_llvm_string(self) -> LLVMString {
        match self {
            BasicMetadataTypeEnum::ArrayType(t) => t.print_to_llvm_string(),
            BasicMetadataTypeEnum::IntType(t) => t.print_to_llvm_string(),
            BasicMetadataTypeEnum::FloatType(t) => t.print_to_llvm_string(),
            BasicMetadataTypeEnum::PointerType(t) => t.print_to_llvm_string(),
            BasicMetadataTypeEnum::StructType(t) => t.print_to_llvm_string(),
            BasicMetadataTypeEnum::VectorType(t) => t.print_to_llvm_string(),
            BasicMetadataTypeEnum::MetadataType(t) => t.print_to_llvm_string(),
        }
    }
}
...
impl<'ctx> AnyTypeEnum<'ctx> {
    pub(crate) unsafe fn new(type_: LLVMTypeRef) -> Self {
        match LLVMGetTypeKind(type_) {
            LLVMTypeKind::LLVMVoidTypeKind => {
                AnyTypeEnum::VoidType(VoidType::new(type_))
            }
            ...
            | LLVMTypeKind::LLVMPPC_FP128TypeKind => {
                AnyTypeEnum::FloatType(FloatType::new(type_))
            }
            #[cfg(any(
                feature = "llvm11-0",
                feature = "llvm12-0",
            ...
            LLVMTypeKind::LLVMLabelTypeKind => {
                panic!("FIXME: Unsupported type: Label")
            }
            LLVMTypeKind::LLVMIntegerTypeKind => {
                AnyTypeEnum::IntType(IntType::new(type_))
            }
            LLVMTypeKind::LLVMFunctionTypeKind => {
                AnyTypeEnum::FunctionType(FunctionType::new(type_))
            }
            LLVMTypeKind::LLVMStructTypeKind => {
                AnyTypeEnum::StructType(StructType::new(type_))
            }
            LLVMTypeKind::LLVMArrayTypeKind => {
                AnyTypeEnum::ArrayType(ArrayType::new(type_))
            }
            LLVMTypeKind::LLVMPointerTypeKind => {
                AnyTypeEnum::PointerType(PointerType::new(type_))
            }
            ...
            LLVMTypeKind::LLVMX86_MMXTypeKind => {
                panic!("FIXME: Unsupported type: MMX")
            }
            #[cfg(any(
                feature = "llvm12-0",
                feature = "llvm13-0",
            ...
            LLVMTypeKind::LLVMX86_AMXTypeKind => {
                panic!("FIXME: Unsupported type: AMX")
            }
            #[cfg(not(any(feature = "llvm3-6", feature = "llvm3-7")))]
            LLVMTypeKind::LLVMTokenTypeKind => {
                panic!("FIXME: Unsupported type: Token")
            }
        }
    }
    ...
    pub fn print_to_llvm_string(self) -> LLVMString {
        match self {
            AnyTypeEnum::ArrayType(t) => t.print_to_llvm_string(),
            AnyTypeEnum::FloatType(t) => t.print_to_llvm_string(),
            AnyTypeEnum::IntType(t) => t.print_to_llvm_string(),
            AnyTypeEnum::PointerType(t) => t.print_to_llvm_string(),
            AnyTypeEnum::StructType(t) => t.print_to_llvm_string(),
            AnyTypeEnum::VectorType(t) => t.print_to_llvm_string(),
            AnyTypeEnum::VoidType(t) => t.print_to_llvm_string(),
            AnyTypeEnum::FunctionType(t) => t.print_to_llvm_string(),
        }
    }
}
...
impl<'ctx> BasicTypeEnum<'ctx> {
    pub(crate) unsafe fn new(type_: LLVMTypeRef) -> Self {
        match LLVMGetTypeKind(type_) {
            ...
            LLVMTypeKind::LLVMIntegerTypeKind => {
                BasicTypeEnum::IntType(IntType::new(type_))
            }
            LLVMTypeKind::LLVMStructTypeKind => {
                BasicTypeEnum::StructType(StructType::new(type_))
            }
            LLVMTypeKind::LLVMPointerTypeKind => {
                BasicTypeEnum::PointerType(PointerType::new(type_))
            }
            LLVMTypeKind::LLVMArrayTypeKind => {
                BasicTypeEnum::ArrayType(ArrayType::new(type_))
            }
            LLVMTypeKind::LLVMVectorTypeKind => {
            ...
            LLVMTypeKind::LLVMMetadataTypeKind => {
                unreachable!("Unsupported basic type: Metadata")
            }
            // see https://llvm.org/docs/LangRef.html#x86-mmx-type
            LLVMTypeKind::LLVMX86_MMXTypeKind => {
                unreachable!("Unsupported basic type: MMX")
            }
            ...
            LLVMTypeKind::LLVMX86_AMXTypeKind => {
                unreachable!("Unsupported basic type: AMX")
            }
            LLVMTypeKind::LLVMLabelTypeKind => {
                unreachable!("Unsupported basic type: Label")
            }
            LLVMTypeKind::LLVMVoidTypeKind => {
                unreachable!("Unsupported basic type: VoidType")
            }
            LLVMTypeKind::LLVMFunctionTypeKind => {
            ...
            #[cfg(not(any(feature = "llvm3-6", feature = "llvm3-7")))]
            LLVMTypeKind::LLVMTokenTypeKind => {
                unreachable!("Unsupported basic type: Token")
            }
        }
    }
    ...
    pub fn const_zero(self) -> BasicValueEnum<'ctx> {
        match self {
            BasicTypeEnum::ArrayType(ty) => {
                ty.const_zero().as_basic_value_enum()
            }
    ...
    pub fn print_to_llvm_string(self) -> LLVMString {
        match self {
            BasicTypeEnum::ArrayType(t) => t.print_to_llvm_string(),
            BasicTypeEnum::FloatType(t) => t.print_to_llvm_string(),
            BasicTypeEnum::IntType(t) => t.print_to_llvm_string(),
            BasicTypeEnum::PointerType(t) => t.print_to_llvm_string(),
            BasicTypeEnum::StructType(t) => t.print_to_llvm_string(),
            BasicTypeEnum::VectorType(t) => t.print_to_llvm_string(),
        }
    }
}
