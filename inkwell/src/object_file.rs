use llvm_sys::object::{
    LLVMDisposeObjectFile, LLVMDisposeRelocationIterator,
    LLVMDisposeSectionIterator, LLVMDisposeSymbolIterator,
    LLVMGetRelocationOffset, LLVMGetRelocationSymbol, LLVMGetRelocationType,
    LLVMGetRelocationTypeName, LLVMGetRelocationValueString,
    LLVMGetRelocations, LLVMGetSectionAddress, LLVMGetSectionContents,
    LLVMGetSectionName, LLVMGetSectionSize, LLVMGetSections,
    LLVMGetSymbolAddress, LLVMGetSymbolName, LLVMGetSymbolSize, LLVMGetSymbols,
    ...
    LLVMMoveToNextSymbol, LLVMObjectFileRef, LLVMRelocationIteratorRef,
...
impl Iterator for SectionIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let at_end = unsafe {
            LLVMIsSectionIteratorAtEnd(self.object_file, self.section_iterator)
                == 1
        };
...
impl Iterator for RelocationIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let relocation =
            Relocation::new(self.relocation_iterator, self.object_file);
...
impl Iterator for SymbolIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let at_end = unsafe {
            LLVMIsSymbolIteratorAtEnd(self.object_file, self.symbol_iterator)
                == 1
        };
