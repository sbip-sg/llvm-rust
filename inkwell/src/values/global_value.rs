...
impl<'ctx> GlobalValue<'ctx> {
    ...
    pub fn get_dll_storage_class(self) -> DLLStorageClass {
        let dll_storage_class =
            unsafe { LLVMGetDLLStorageClass(self.as_value_ref()) };
    ...
    pub fn get_thread_local_mode(self) -> Option<ThreadLocalMode> {
        let thread_local_mode =
            unsafe { LLVMGetThreadLocalMode(self.as_value_ref()) };
        ...
        unsafe {
            if has_unnamed_addr {
                LLVMSetUnnamedAddress(
                    self.as_value_ref(),
                    UnnamedAddress::Global.into(),
                )
            } else {
                LLVMSetUnnamedAddress(
                    self.as_value_ref(),
                    UnnamedAddress::None.into(),
                )
            }
        }
    }
    ...
    pub fn get_unnamed_address(self) -> UnnamedAddress {
        ...
        let unnamed_address =
            unsafe { LLVMGetUnnamedAddress(self.as_value_ref()) };
...
impl<'ctx> Display for GlobalValue<'ctx> {
