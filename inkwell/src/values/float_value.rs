use llvm_sys::core::{
    LLVMConstFAdd, LLVMConstFCmp, LLVMConstFDiv, LLVMConstFMul, LLVMConstFNeg,
    LLVMConstFPCast, LLVMConstFPExt, LLVMConstFPToSI, LLVMConstFPToUI,
    LLVMConstFPTrunc, LLVMConstFRem, LLVMConstFSub, LLVMConstRealGetDouble,
};
...
impl<'ctx> FloatValue<'ctx> {
    ...
    pub fn is_instruction(self) -> bool {
        self.float_value.is_instruction()
    }
    ...
    pub fn const_to_unsigned_int(
        self,
        int_type: IntType<'ctx>,
    ) -> IntValue<'ctx> {
        unsafe {
            IntValue::new(LLVMConstFPToUI(
                self.as_value_ref(),
    ...
    pub fn const_to_signed_int(
        self,
        int_type: IntType<'ctx>,
    ) -> IntValue<'ctx> {
        unsafe {
            IntValue::new(LLVMConstFPToSI(
                self.as_value_ref(),
                int_type.as_type_ref(),
            ))
        }
    ...
    pub fn get_constant(self) -> Option<(f64, bool)> {
        ...
        let mut lossy = 0;
        let constant =
            unsafe { LLVMConstRealGetDouble(self.as_value_ref(), &mut lossy) };
...
impl<'ctx> AsValueRef for FloatValue<'ctx> {
    fn as_value_ref(&self) -> LLVMValueRef {
        self.float_value.value
    }
}

impl<'ctx> Display for FloatValue<'ctx> {
