...
use llvm_sys::core::{
    LLVMGetElementType, LLVMGetReturnType, LLVMGetTypeKind, LLVMTypeOf,
};
...
impl<'ctx> CallableValue<'ctx> {
    ...
    /// Convert the current `CallableValue` into the `PointerValue` type.
    ...
    /// Convert the current `CallableValue` into the `FunctionValue` type.
    pub fn as_function_value(&self) -> Option<FunctionValue<'ctx>> {
        match self.0 {
            Left(function) => Some(function),
            Right(_) => None,
        }
    }

    /// Convert the current `CallableValue` into the `PointerValue` type.
    pub fn as_pointer_value(&self) -> Option<PointerValue<'ctx>> {
        match self.0 {
            Left(_) => None,
            Right(pointer) => Some(pointer),
        }
    }
...
impl<'ctx> TryFrom<PointerValue<'ctx>> for CallableValue<'ctx> {
    ...
    fn try_from(value: PointerValue<'ctx>) -> Result<Self, Self::Error> {
        ...
        let ty_kind = unsafe {
            LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(value_ref)))
        };
...
impl<'ctx> Display for CallableValue<'ctx> {
