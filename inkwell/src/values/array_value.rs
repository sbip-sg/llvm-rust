...
impl<'ctx> ArrayValue<'ctx> {
    ...
    /// Determines whether or not this value is an `InstructionValue`.
    pub fn is_instruction(self) -> bool {
        self.array_value.is_instruction()
    }
...
impl<'ctx> Display for ArrayValue<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.print_to_llvm_string())
    }
}

impl<'ctx> fmt::Debug for ArrayValue<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let is_const_array =
            unsafe { !LLVMIsAConstantArray(self.as_value_ref()).is_null() };
