...
use crate::values::{
    ArrayValue, FloatValue, FunctionValue, InstructionValue, IntValue,
...
macro_rules! enum_value_set {
    ($enum_name:ident: $($args:ident),*) => (
        ...
        impl<'ctx> AsValueRef for $enum_name<'ctx> {
...
enum_value_set! {AnyValueEnum: ArrayValue, IntValue, FloatValue, FunctionValue, PointerValue, StructValue, VectorValue, InstructionValue, MetadataValue}
    ...
    pub(crate) unsafe fn new(value: LLVMValueRef) -> Self {
        match LLVMGetTypeKind(LLVMTypeOf(value)) {
            ...
            LLVMTypeKind::LLVMIntegerTypeKind => {
                AnyValueEnum::IntValue(IntValue::new(value))
            }
            LLVMTypeKind::LLVMStructTypeKind => {
                AnyValueEnum::StructValue(StructValue::new(value))
            }
            LLVMTypeKind::LLVMPointerTypeKind => {
                AnyValueEnum::PointerValue(PointerValue::new(value))
            }
            LLVMTypeKind::LLVMArrayTypeKind => {
                AnyValueEnum::ArrayValue(ArrayValue::new(value))
            }
            LLVMTypeKind::LLVMVectorTypeKind => {
                AnyValueEnum::VectorValue(VectorValue::new(value))
            }
...
impl<'ctx> BasicValueEnum<'ctx> {
    pub unsafe fn new(value: LLVMValueRef) -> Self {
        match LLVMGetTypeKind(LLVMTypeOf(value)) {
            ...
            LLVMTypeKind::LLVMIntegerTypeKind => {
                BasicValueEnum::IntValue(IntValue::new(value))
            }
            LLVMTypeKind::LLVMStructTypeKind => {
    ...
    /// Set the name of a `BasicValueEnum`.
    ...
    pub fn is_instruction(&self) -> bool {
        match self {
            BasicValueEnum::ArrayValue(v) => v.is_instruction(),
            BasicValueEnum::IntValue(v) => v.is_instruction(),
            BasicValueEnum::FloatValue(v) => v.is_instruction(),
            BasicValueEnum::PointerValue(v) => v.is_instruction(),
            BasicValueEnum::StructValue(v) => v.is_instruction(),
            BasicValueEnum::VectorValue(v) => v.is_instruction(),
        }
    }
    ...
    pub fn into_instruction(&self) -> Option<InstructionValue<'ctx>> {
        match self {
...
impl<'ctx> Display for AggregateValueEnum<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.print_to_llvm_string())
    }
}

impl<'ctx> Display for AnyValueEnum<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.print_to_llvm_string())
    }
}

impl<'ctx> Display for BasicValueEnum<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.print_to_llvm_string())
    }
}

impl<'ctx> Display for BasicMetadataValueEnum<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.print_to_llvm_string())
    }
}
