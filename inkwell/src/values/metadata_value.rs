use llvm_sys::core::{
    LLVMGetMDNodeNumOperands, LLVMGetMDNodeOperands, LLVMGetMDString,
    LLVMIsAMDNode, LLVMIsAMDString,
};
...
impl<'ctx> MetadataValue<'ctx> {
    pub(crate) unsafe fn new(value: LLVMValueRef) -> Self {
        ...
        assert!(
            !LLVMIsAMDNode(value).is_null()
                || !LLVMIsAMDString(value).is_null()
        );
...
impl<'ctx> AsValueRef for MetadataValue<'ctx> {
    fn as_value_ref(&self) -> LLVMValueRef {
        self.metadata_value.value
    }
}

impl<'ctx> Display for MetadataValue<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.print_to_llvm_string())
    }
}

impl<'ctx> Debug for MetadataValue<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        d.field("repr", &self.print_to_llvm_string());
