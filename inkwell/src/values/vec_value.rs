use llvm_sys::core::{
    LLVMConstExtractElement, LLVMConstInsertElement, LLVMConstSelect,
    LLVMConstShuffleVector, LLVMGetAsString, LLVMGetElementAsConstant,
    LLVMIsAConstantDataVector, LLVMIsAConstantVector, LLVMIsConstantString,
};
...
impl<'ctx> VectorValue<'ctx> {
    ...
    pub fn get_element_as_constant(self, index: u32) -> BasicValueEnum<'ctx> {
        unsafe {
            BasicValueEnum::new(LLVMGetElementAsConstant(
                self.as_value_ref(),
                index,
            ))
        }
...
impl<'ctx> AsValueRef for VectorValue<'ctx> {
    fn as_value_ref(&self) -> LLVMValueRef {
        self.vec_value.value
    }
}

impl<'ctx> Display for VectorValue<'ctx> {
