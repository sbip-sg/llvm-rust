...
pub use crate::values::basic_block::BasicBlock;
...
use llvm_sys::core::{
    LLVMDumpValue, LLVMGetFirstUse, LLVMIsAFunction, LLVMIsAInstruction,
    LLVMIsConstant, LLVMIsNull, LLVMIsUndef, LLVMPrintTypeToString,
...
impl<'ctx> Value<'ctx> {
    ...
    fn is_function(self) -> bool {
        unsafe { !LLVMIsAFunction(self.value).is_null() }
    }

    fn as_function(self) -> Option<FunctionValue<'ctx>> {
        if !self.is_function() {
            return None;
        }

        unsafe { FunctionValue::new(self.value) }
    }
    ...
    fn print_to_llvm_string(self) -> LLVMString {
    ...
    /// Get first use of the current `Value`.
    ...
    /// Get all users of the current `Value`.
