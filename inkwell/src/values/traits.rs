...
use crate::types::{
    FloatMathType, FloatType, IntMathType, IntType, PointerMathType,
    PointerType, VectorType,
};
use crate::values::{
    AggregateValueEnum, AnyValueEnum, ArrayValue, BasicValueEnum,
    BasicValueUse, CallSiteValue, FloatValue, FunctionValue, GlobalValue,
...
pub trait AnyValue<'ctx>: AsValueRef + Debug {
    ...
    fn print_to_llvm_string(&self) -> LLVMString {
        unsafe { Value::new(self.as_value_ref()).print_to_llvm_string() }
    }

    /// Prints a value to a pretty `String`
    fn print_to_string(&self) -> String {
        self.print_to_llvm_string().to_string().trim().to_string()
    }
}
