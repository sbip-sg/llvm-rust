...
use either::Either;
use llvm_sys::core::{
    LLVMGetInstructionCallConv, LLVMGetTypeKind, LLVMIsTailCall,
    LLVMSetInstrParamAlignment, LLVMSetInstructionCallConv, LLVMSetTailCall,
    LLVMTypeOf,
};
...
impl<'ctx> AsValueRef for CallSiteValue<'ctx> {
    fn as_value_ref(&self) -> LLVMValueRef {
        self.0.value
    }
}

impl<'ctx> Display for CallSiteValue<'ctx> {
