use llvm_sys::analysis::{
    LLVMVerifierFailureAction, LLVMVerifyFunction, LLVMViewFunctionCFG,
    LLVMViewFunctionCFGOnly,
};
#[llvm_versions(3.9..=latest)]
use llvm_sys::core::{
    LLVMAddAttributeAtIndex, LLVMGetAttributeCountAtIndex,
    LLVMGetEnumAttributeAtIndex, LLVMGetStringAttributeAtIndex,
...
impl<'ctx> FunctionValue<'ctx> {
    ...
    pub fn get_basic_blocks(self) -> Vec<BasicBlock<'ctx>> {
        ...
        let mut raw_vec: Vec<LLVMBasicBlockRef> =
    ...
    pub fn get_first_use(self) -> Option<BasicValueUse<'ctx>> {
        self.fn_value.get_first_use()
    }

    /// Get all users of the current `FunctionValue`.
    pub fn get_all_users(self) -> Vec<AnyValueEnum<'ctx>> {
        self.fn_value.get_all_users()
    }
    ...
    #[cfg(feature = "llvm3-8")]
    pub unsafe fn get_personality_function(
        self,
    ...
    pub fn attributes(self, loc: AttributeLoc) -> Vec<Attribute> {
        ...
        let mut attribute_refs: Vec<MaybeUninit<Attribute>> =
            vec![MaybeUninit::uninit(); count];
    ...
    pub fn set_subprogram(self, subprogram: DISubprogram<'ctx>) {
        unsafe {
            LLVMSetSubprogram(self.as_value_ref(), subprogram.metadata_ref)
        }
    }
...
impl<'ctx> Display for FunctionValue<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.print_to_string())
    }
}

impl<'ctx> fmt::Debug for FunctionValue<'ctx> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let llvm_value = self.print_to_llvm_string();
        ...
        f.debug_struct("FunctionValue")
            ...
            .field("llvm_type", &llvm_type.print_to_llvm_string())
            .finish()
    }
}
...
impl<'ctx> Iterator for ParamValueIter<'ctx> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.start {
            let first_value =
                unsafe { LLVMGetFirstParam(self.param_iter_value) };
