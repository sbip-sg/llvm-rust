...
use llvm_sys::core::{
    LLVMAPIntGetAllWords, LLVMAPIntGetNumWords, LLVMConstAShr, LLVMConstAdd,
    LLVMConstAnd, LLVMConstBitCast, LLVMConstExactSDiv, LLVMConstICmp,
    LLVMConstIntCast, LLVMConstIntGetSExtValue, LLVMConstIntGetZExtValue,
    LLVMConstIntToPtr, LLVMConstLShr, LLVMConstMul, LLVMConstNSWAdd,
    LLVMConstNSWMul, LLVMConstNSWNeg, LLVMConstNSWSub, LLVMConstNUWAdd,
    LLVMConstNUWMul, LLVMConstNUWNeg, LLVMConstNUWSub, LLVMConstNeg,
    LLVMConstNot, LLVMConstOr, LLVMConstSDiv, LLVMConstSExt,
    LLVMConstSExtOrBitCast, LLVMConstSIToFP, LLVMConstSRem, LLVMConstSelect,
    LLVMConstShl, LLVMConstSub, LLVMConstTrunc, LLVMConstTruncOrBitCast,
    LLVMConstUDiv, LLVMConstUIToFP, LLVMConstURem, LLVMConstXor, LLVMConstZExt,
    LLVMConstZExtOrBitCast, LLVMIsAConstantInt,
};
...
impl<'ctx> IntValue<'ctx> {
    ...
    pub fn const_exact_signed_div(self, rhs: IntValue<'ctx>) -> Self {
        unsafe {
            IntValue::new(LLVMConstExactSDiv(
                self.as_value_ref(),
                rhs.as_value_ref(),
            ))
        }
    ...
    pub fn const_exact_unsigned_div(self, rhs: IntValue<'ctx>) -> Self {
        unsafe {
            IntValue::new(LLVMConstExactUDiv(
                self.as_value_ref(),
                rhs.as_value_ref(),
            ))
        }
    ...
    pub fn const_unsigned_to_float(
        self,
    ...
    pub fn const_truncate_or_bit_cast(
        self,
    ...
    // TODO: More descriptive name
    pub fn const_s_extend_or_bit_cast(
        self,
        int_type: IntType<'ctx>,
    ) -> IntValue<'ctx> {
        unsafe {
    ...
    // TODO: More descriptive name
    pub fn const_z_ext_or_bit_cast(
        self,
        int_type: IntType<'ctx>,
    ) -> IntValue<'ctx> {
        unsafe {
    ...
    /// Obtains a big integer constant from `IntValue`.
    pub fn get_big_int_constant(self) -> Option<BigInt> {
        if !self.is_constant_int() {
            return None;
        }
        ...
        unsafe {
            let mut size = 0;
            let words = LLVMAPIntGetAllWords(self.as_value_ref(), &mut size);
            let digits =
                slice::from_raw_parts(words as *mut u32, (2 * size) as usize);
            let big_uint = BigUint::from_slice(digits);
            let res = BigInt::from_biguint(Sign::NoSign, big_uint);
            Some(res)
        }
    }
...
impl<'ctx> AsValueRef for IntValue<'ctx> {
    fn as_value_ref(&self) -> LLVMValueRef {
        self.int_value.value
    }
}

impl<'ctx> Display for IntValue<'ctx> {
