...
fn test_get_function() {
    ...
    assert!(module.get_functions().count() == 0);
...
fn test_garbage_ir_fails_create_module_from_ir() {
    let context = Context::create();
    let memory_buffer =
        MemoryBuffer::create_from_memory_range(b"garbage ir data", "my_ir");
...
fn test_parse_from_buffer() {
    ...
    let garbage_buffer =
        MemoryBuffer::create_from_memory_range(b"garbage ir data", "my_ir");
    let module_result =
        Module::parse_bitcode_from_buffer(&garbage_buffer, &context);
...
fn test_parse_from_path() {
    ...
    let module_result =
        Module::parse_bitcode_from_path(&garbage_path, &context);
...
fn test_clone() {
    ...
    assert_eq!(
        module.print_to_llvm_string(),
        module2.print_to_llvm_string()
    );
}
