...
fn test_operands() {
    ...
    let free_operand0 =
        free_instruction.get_operand(0).unwrap().left().unwrap();
    let free_operand1 =
        free_instruction.get_operand(1).unwrap().left().unwrap();
    let free_operand0_instruction =
        free_operand0.as_instruction_value().unwrap();
...
fn test_basic_block_operand() {
    ...
    let bb_operand =
        branch_instruction.get_operand(0).unwrap().right().unwrap();
...
fn test_instructions() {
    ...
    let f32_ptr_type = f32_type.ptr_type(AddressSpace::Generic);
    let fn_type =
        void_type.fn_type(&[f32_ptr_type.into(), f32_type.into()], false);
    ...
    let icmp =
        builder.build_int_compare(IntPredicate::EQ, ptr_val, ptr_val, "icmp");
    let f32_sum = builder.build_float_add(arg2, f32_val, "f32_sum");
    let fcmp =
        builder.build_float_compare(FloatPredicate::OEQ, f32_sum, arg2, "fcmp");
    let free_instruction = builder.build_free(arg1);
    let return_instruction = builder.build_return(None);
    ...
    assert_eq!(instruction_clone, return_instruction);
...
fn test_volatile_atomicrmw_cmpxchg() {
    ...
    let fn_type =
        void_type.fn_type(&[i32_ptr_type.into(), i32_type.into()], false);
...
fn test_metadata_kinds() {
    ...
    let struct_type =
        context.struct_type(&[i8_type.into(), f32_type.into()], false);
