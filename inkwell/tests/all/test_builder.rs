...
use inkwell::{
    AddressSpace, AtomicOrdering, AtomicRMWBinOp, OptimizationLevel,
};
...
fn test_build_invoke_cleanup_resume() {
    ...
    let call_site =
        builder.build_invoke(function, &[], then_block, catch_block, "get_pi");
...

    {
        ...
        let exception_type =
            context.struct_type(&[i8_ptr_type.into(), i32_type.into()], false);
...
fn landing_pad_filter() {
...

    {
        ...
        let type_info_int = module.add_global(
            i8_ptr_type,
        ...
        let filter_pattern = i8_ptr_type.const_array(&[type_info_int
            .as_any_value_enum()
            .into_pointer_value()]);
        builder.build_landing_pad(
            exception_type,
...
fn test_null_checked_ptr_ops() {
    ...
    let new_ptr =
        builder.build_int_to_ptr(new_ptr_as_int, i8_ptr_type, "int_as_ptr");
    ...
    unsafe {
        let check_null_index1 = execution_engine
            .get_function::<unsafe extern "C" fn(*const i8) -> i8>(
                "check_null_index1",
            )
            .unwrap();
...
fn test_bit_shifts() {
    ...
    // }
    let fn_value =
        module.add_function("right_shift_sign_extend", fn_type, None);
    let value = fn_value.get_first_param().unwrap().into_int_value();
    let bits = fn_value.get_nth_param(1).unwrap().into_int_value();
    ...
    unsafe {
        ...
        let right_shift_sign_extend = execution_engine
            .get_function::<unsafe extern "C" fn(i8, u8) -> i8>(
                "right_shift_sign_extend",
            )
            .unwrap();
...
fn test_no_builder_double_free2() {
    ...
    // assert_eq!(*module.print_to_llvm_string(), *CString::new("; ModuleID = \'my_mod\'\n\ndefine void @my_fn() {\nentry:\n  unreachable\n}\n").unwrap());
...
fn test_vector_convert_ops() {
    ...
    let casted_vec =
        builder.build_int_cast(in_vec, int32_vec_type, "casted_vec");
    let _uncasted_vec =
        builder.build_int_cast(casted_vec, int8_vec_type, "uncasted_vec");
    ...
    let in_vec = fn_value.get_first_param().unwrap().into_vector_value();
    let casted_vec =
        builder.build_float_cast(in_vec, float16_vec_type, "casted_vec");
    let _uncasted_vec =
        builder.build_float_cast(casted_vec, float32_vec_type, "uncasted_vec");
    builder.build_return(Some(&casted_vec));
    assert!(fn_value.verify(true));
    ...
    let fn_value =
        module.add_function("test_float_to_int_vec_cast", fn_type, None);
    let entry = context.append_basic_block(fn_value, "entry");
    let builder = context.create_builder();
    ...
    let in_vec = fn_value.get_first_param().unwrap().into_vector_value();
    let casted_vec =
        builder.build_float_to_signed_int(in_vec, int32_vec_type, "casted_vec");
    let _uncasted_vec = builder.build_signed_int_to_float(
        casted_vec,
        float32_vec_type,
        "uncasted_vec",
    );
...
fn test_vector_convert_ops_respect_target_signedness() {
    ...
    builder.position_at_end(entry);
    let in_vec = fn_value.get_first_param().unwrap().into_vector_value();
    let casted_vec = builder.build_int_cast_sign_flag(
        in_vec,
        int8_vec_type,
        true,
        "casted_vec",
    );
    let _uncasted_vec = builder.build_int_cast_sign_flag(
        casted_vec,
        int8_vec_type,
        true,
        "uncasted_vec",
...
fn test_vector_binary_ops() {
    ...
    let p3_vec = fn_value.get_nth_param(2).unwrap().into_vector_value();
    let multiplied_vec =
        builder.build_float_mul(p1_vec, p2_vec, "multipled_vec");
    let divided_vec =
        builder.build_float_div(multiplied_vec, p3_vec, "divided_vec");
    ...
    let multiplied_vec =
        builder.build_int_mul(compared_vec, p3_vec, "multiplied_vec");
...
fn test_insert_value() {
    ...
    let i32_type = context.i32_type();
    let struct_type =
        context.struct_type(&[i32_type.into(), f32_type.into()], false);
    ...
    let const_float = f32_type.const_float(std::f64::consts::PI);
...
fn run_memcpy_on<'ctx>(
    ...
    for index in 0..4 {
        ...
        let elem_ptr = unsafe {
            builder.build_in_bounds_gep(array_ptr, &[index_val], "index")
        };
    ...
    builder
        .build_memcpy(dest_ptr, alignment, array_ptr, alignment, size_val)?;
...
fn run_memmove_on<'ctx>(
    ...
    builder
        .build_memmove(dest_ptr, alignment, array_ptr, alignment, size_val)?;
...
fn run_memset_on<'ctx>(
    ...
    let part_2 =
        unsafe { builder.build_in_bounds_gep(array_ptr, &[index], "index") };
...
fn test_bitcast() {
    ...
    assert!(
        module.verify().is_ok(),
        "{}",
        module.print_to_llvm_string().to_string()
    );
...
fn test_safe_struct_gep() {
    ...
    let fn_type =
        void_type.fn_type(&[i32_ptr_ty.into(), struct_ptr_ty.into()], false);
    let fn_value = module.add_function("", fn_type, None);
    let entry = context.append_basic_block(fn_value, "entry");
