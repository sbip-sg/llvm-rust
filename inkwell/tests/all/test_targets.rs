...
use inkwell::targets::{
    ByteOrdering, CodeModel, FileType, InitializationConfig, RelocMode, Target,
    TargetData, TargetMachine, TargetTriple,
};
...
fn test_target_and_target_machine() {
    ...
    let good_target2 =
        Target::from_triple(&TargetTriple::create("x86_64-pc-linux-gnu"));
...
fn test_ptr_sized_int() {
    ...
    let int_type2 =
        context.ptr_sized_int_type(target_data, Some(address_space));
