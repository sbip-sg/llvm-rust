...
use inkwell::{
    AddressSpace, DLLStorageClass, FloatPredicate, GlobalVisibility,
    ThreadLocalMode,
};
...
fn test_set_get_name() {
    ...
    let struct_val =
        context.const_struct(&[i8_val.into(), f128_val.into()], false);
...
fn test_undef() {
    ...
    let struct_undef =
        context.struct_type(&[bool_type.into()], false).get_undef();
...
fn test_metadata() {
    ...
    )))]
    {
        ...
        let md_node_child =
            context.metadata_node(&[bool_val.into(), f32_val.into()]);
        ...
        let _ = ret_instr.set_metadata(ret_instr_md, 2);

        assert!(ret_instr.has_metadata());
        assert!(ret_instr.get_metadata(1).is_none());

        let md_node_values =
            ret_instr.get_metadata(2).unwrap().get_node_values();
        ...
        let context_metadata_node =
            context.metadata_node(&[bool_val.into(), f32_val.into()]);
        let context_metadata_string =
            context.metadata_string("my_context_metadata");
...
fn test_floats() {
    ...
    assert_eq!(
        neg_two.print_to_llvm_string().to_str(),
    ...
    assert_eq!(
        neg_three.print_to_llvm_string().to_str(),
    ...
    assert_eq!(
        pos_six.print_to_llvm_string().to_str(),
    ...
    assert_eq!(
        pos_eight.print_to_llvm_string().to_str(),
    ...
    assert_eq!(
        pos_four.print_to_llvm_string().to_str(),
    ...
    assert_eq!(
        rem.print_to_llvm_string().to_str(),
        Ok("double 2.000000e+00")
    );
...
fn test_function_value_no_params() {
    ...
    assert_eq!(fn_value.get_param_iter().count(), 0);
...
fn test_value_from_string() {
    ...
    assert_eq!(i8_val.print_to_llvm_string().to_str(), Ok("i8 121"));

    let i8_val = i8_type
        .const_int_from_string("0121", StringRadix::try_from(10).unwrap())
        .unwrap();

    assert_eq!(i8_val.print_to_llvm_string().to_string(), "i8 121");
    ...
    assert_eq!(
        f64_val.print_to_llvm_string().to_string(),
        "double 3.600000e+00"
    );
...
fn test_global_byte_array() {
    ...
    let global_string = module.add_global(
        i8_array_type,
...
fn test_globals() {
    ...
    let global2 =
        module.add_global(i8_type, Some(AddressSpace::Const), "my_global2");
...
fn test_phi_values() {
    ...
    assert_eq!(
        phi.print_to_llvm_string().to_str(),
...
fn test_allocations() {
    ...
    assert_eq!(
        stack_ptr.get_type().print_to_llvm_string().to_str(),
        Ok("i32*")
    );

    let stack_array =
        builder.build_array_alloca(i32_type, i32_three, "stack_array");

    assert_eq!(
        stack_array.get_type().print_to_llvm_string().to_str(),
        Ok("i32*")
    );
    ...
    assert_eq!(
        heap_ptr.unwrap().get_type().print_to_llvm_string().to_str(),
        Ok("i32*")
    );

    let heap_array =
        builder.build_array_malloc(i32_type, i32_three, "heap_array");
    ...
    let bad_array_malloc_res =
        builder.build_array_malloc(unsized_type, i32_three, "");
...
fn test_string_values() {
    ...
    assert_eq!(
        string.print_to_llvm_string().to_string(),
        "[9 x i8] c\"my_string\""
    );
    assert_eq!(
        string_null.print_to_llvm_string().to_string(),
    ...
    let non_string_vec_i32 =
        VectorType::const_vector(&[i32_val, i32_val2, i32_val2]);
...
fn test_consts() {
    ...
    let arbitrary_precision_int =
        i64_type.const_int_arbitrary_precision(&[1, 2]);
    ...
    assert_eq!(
        arbitrary_precision_int.print_to_llvm_string().to_str(),
        Ok("i64 1")
    );
...
fn test_vectors() {
    ...
    let extracted =
        builder.build_extract_element(vector_param, i32_zero, "extract");
...
fn test_aggregate_returns() {
    ...
    let i32_seven = i32_type.const_int(7, false);
    let struct_type =
        context.struct_type(&[i32_type.into(), i32_type.into()], false);
    let fn_type =
        struct_type.fn_type(&[i32_ptr_type.into(), i32_ptr_type.into()], false);
    let fn_value = module.add_function("my_func", fn_type, None);
    let bb = context.append_basic_block(fn_value, "entry");
...
fn test_constant_expression() {
    ...
    let expr = builder.build_ptr_to_int(
