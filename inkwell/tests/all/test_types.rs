...
fn test_struct_type() {
    ...
    let av_struct =
        context.struct_type(&[int_vector.into(), float_array.into()], false);
    ...
    let av_struct =
        context.struct_type(&[int_vector.into(), float_array.into()], true);
    ...
    assert!(
        opaque_struct.set_body(&[int_vector.into(), float_array.into()], true)
    );
    ...
    {
        let field_1 =
            no_longer_opaque_struct.get_field_type_at_index(0).unwrap();
        let field_2 =
            no_longer_opaque_struct.get_field_type_at_index(1).unwrap();
...
fn sized_types(global_ctx: &Context) {
    ...
    let struct_type =
        global_ctx.struct_type(&[i8_type.into(), f128_type.into()], false);
    let struct_type2 = global_ctx.struct_type(&[], false);
    let struct_type3 =
        global_ctx.struct_type(&[i8_type.into(), f128_type.into()], true);
    let struct_type4 = global_ctx.struct_type(&[], true);
    let fn_type = void_type.fn_type(&[], false);
    let fn_type2 = i8_type.fn_type(&[], false);
    let fn_type3 =
        void_type.fn_type(&[i32_type.into(), struct_type.into()], false);
    let fn_type4 =
        i8_type.fn_type(&[struct_type.into(), i32_type.into()], false);
...
fn test_const_zero() {
    ...
    let struct_type =
        context.struct_type(&[i8_type.into(), f128_type.into()], false);
    ...
    assert_eq!(bool_zero.print_to_llvm_string().to_str(), Ok("i1 false"));
    assert_eq!(i8_zero.print_to_llvm_string().to_str(), Ok("i8 0"));
    assert_eq!(i16_zero.print_to_llvm_string().to_str(), Ok("i16 0"));
    assert_eq!(i32_zero.print_to_llvm_string().to_str(), Ok("i32 0"));
    assert_eq!(i64_zero.print_to_llvm_string().to_str(), Ok("i64 0"));
    assert_eq!(i128_zero.print_to_llvm_string().to_str(), Ok("i128 0"));
    assert_eq!(f16_zero.print_to_llvm_string().to_str(), Ok("half 0xH0000"));
    assert_eq!(
        f32_zero.print_to_llvm_string().to_str(),
        Ok("float 0.000000e+00")
    );
    assert_eq!(
        f64_zero.print_to_llvm_string().to_str(),
        Ok("double 0.000000e+00")
    );
    assert_eq!(
        f80_zero.print_to_llvm_string().to_str(),
        Ok("x86_fp80 0xK00000000000000000000")
    );
    assert_eq!(
        f128_zero.print_to_llvm_string().to_str(),
        Ok("fp128 0xL00000000000000000000000000000000")
    );
    assert_eq!(
        ppc_f128_zero.print_to_llvm_string().to_str(),
        Ok("ppc_fp128 0xM00000000000000000000000000000000")
    );
    assert_eq!(
        struct_zero.print_to_llvm_string().to_str(),
        Ok("{ i8, fp128 } zeroinitializer")
    );
    assert_eq!(ptr_zero.print_to_llvm_string().to_str(), Ok("double* null"));
    assert_eq!(
        vec_zero.print_to_llvm_string().to_str(),
        Ok("<42 x double> zeroinitializer")
    );
    assert_eq!(
        array_zero.print_to_llvm_string().to_str(),
        Ok("[42 x double] zeroinitializer")
    );
}
