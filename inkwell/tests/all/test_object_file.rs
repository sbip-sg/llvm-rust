...
fn test_section_iterator() {
    ...
    gv_b.set_initializer(
        &context.i16_type().const_zero().as_basic_value_enum(),
    );
    gv_b.set_section("B");

    let gv_c = module.add_global(context.i32_type(), None, "c");
    gv_c.set_initializer(
        &context.i32_type().const_zero().as_basic_value_enum(),
    );
    gv_c.set_section("C");

    let func =
        module.add_function("d", context.void_type().fn_type(&[], false), None);
    func.set_section("D");
...
fn test_section_contains_nul() {
    ...
    for section in object_file.get_sections() {
        if section.get_name().and_then(|name| name.to_str().ok())
            == Some("test")
        {
