...
use inkwell::debug_info::{
    AsDIScope, DIFlags, DIFlagsConstants, DISubprogram, DWARFEmissionKind,
    DWARFSourceLanguage,
};
...
fn test_struct_with_placeholders() {
    ...
    let member_placeholders = member_types
        .iter()
        .map(|_ty| unsafe {
            dibuilder.create_placeholder_derived_type(&context)
        })
        .collect::<Vec<_>>();
...
fn test_global_expressions() {
    ...
    let di_type =
        dibuilder.create_basic_type("type_name", 0_u64, 0x00, DIFlags::ZERO);
    ...
    let metadata =
        context.metadata_node(&[gv_debug.as_metadata_value(&context).into()]);
    ...
    assert!(
        gv.print_to_llvm_string().to_string().contains("!dbg"),
        "expected !dbg but generated gv was {}",
        gv.print_to_llvm_string()
    );
}
