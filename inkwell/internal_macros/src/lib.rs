...
// TRUNG: allow all clippy lints in Inkwell to override Verazt's settings
#![allow(clippy::all)]
...
const FEATURE_VERSIONS: [&str; 15] = [
    "llvm3-6", "llvm3-7", "llvm3-8", "llvm3-9", "llvm4-0", "llvm5-0",
    "llvm6-0", "llvm7-0", "llvm8-0", "llvm9-0", "llvm10-0", "llvm11-0",
    "llvm12-0", "llvm13-0", "llvm14-0",
];
...
fn get_features(vt: VersionType) -> Result<Vec<&'static str>> {
    ...
    match vt {
        ...
        VersionType::InclusiveRange((start, start_span), (end, end_span)) => {
            ...
            let start_index =
                get_feature_index(&features, start_feature, start_span)?;
            let end_index =
                get_feature_index(&features, end_feature, end_span)?;
        ...
        if lookahead.peek(LitFloat) {
            ...
            if lookahead.peek(Token![..=]) {
                ...
                if lookahead.peek(Ident) {
                    ...
                    if to == "latest" {
                        Ok(VersionType::InclusiveRangeToLatest(
                            from_val,
                            from.span(),
                        ))
                    } else {
                ...
                if lookahead.peek(Ident) {
                    let to = input.parse::<Ident>().unwrap();
                    if to == "latest" {
                        Ok(VersionType::ExclusiveRangeToLatest(
                            from_val,
                            from.span(),
                        ))
                    } else {
...
pub fn llvm_versions(
    attribute_args: TokenStream,
    attributee: TokenStream,
) -> TokenStream {
...
/// ```
#[proc_macro_attribute]
pub fn llvm_versioned_item(
    _attribute_args: TokenStream,
    attributee: TokenStream,
) -> TokenStream {
    let attributee = parse_macro_input!(attributee as Item);
...
impl EnumVariant {
    fn new(variant: &Variant) -> Self {
        ...
        let llvm_variant = Ident::new(
            &format!("LLVM{}", rust_variant.to_string()),
            variant.span(),
        );
